...
export default class EditFocusListModal extends LightningElement {
    isLoaded = false;
    
    @api recordId;
    @api objectApiName; // ← REQUIRED for quick actions
    ...
    @track pendingSubmitAction = null;
    @track b1Questions = [
    ...
    // Wire to get current page reference for recordId fallback
    @wire(CurrentPageReference)
    wiredPageRef(pageRef) {
        if (pageRef && !this.recordId) {
            // Fallback to page reference attributes for record pages (not state, as it's in attributes)
            this.recordId = pageRef.attributes.recordId || pageRef.state.c__recordId || pageRef.state.recordId || null;
            if (this.recordId) {
                this.loadOpportunityIfIdPresent();
            }
        }
    }
    ...
    connectedCallback() {
        // The quick-action panel fires this event **after** it is rendered.
        this.addEventListener('lightning__recordcontext', this.handleRecordContext);
        // If the component is on a record page, recordId is already there.
        this.loadOpportunityIfIdPresent();
    }
    disconnectedCallback() {
        this.removeEventListener('lightning__recordcontext', this.handleRecordContext);
    }
    /* ------------------------------------------------------------------ */
    /* 2. Event handler – receives the record Id from the quick action */
    /* ------------------------------------------------------------------ */
    handleRecordContext = (event) => {
    ...
    /* ------------------------------------------------------------------ */
    /* 3. Centralised load logic (used both from page & quick-action) */
    /* ------------------------------------------------------------------ */
    loadOpportunityIfIdPresent() {
        console.log('edit screen recordID==>' + this.recordId);
        if (this.recordId) {
...
handleSecondScreenCommentChange(event) {
    ...
    return questionsArray.map(q => {
        ...
        return {
            ...q,
            commentErrorClass: hasError ? 'invalid-field' : ''
        };
    });
}
    reloadRecordPage() {
    ...
    closeModal() {
        ...
        this.dispatchEvent(new CloseActionScreenEvent()); // This closes the Quick Action
...
sendEmailAndClose() {
  ...
  renderedCallback() {
        if (!this.isLoaded) {
            ...
            style.textContent = `
            /* Target the modal container with high specificity */
        ...
        min-width: 40rem !important;
            ...
            /* Target the actual modal content area */
            ...
            /* Table container should be responsive */
            .table-container {
                width: 100% !important;
                overflow-x: auto !important;
            }
           
            /* Header row should use full width */
            .header-row {
                width: 100% !important;
            }
        `;
            // Append to the document head for global scope
